...
(function() {
  ...
  PusherClient = (function(_super) {
    ...
    PusherClient.prototype.unsubscribe = function(channel_name, channel_data) {
      ...
      logger('info', 'Pusher', "Unsubscribing from " + channel_name);
      stringToSign = "" + this.state.socket_id + ":" + channel_name + ":" + (JSON.stringify(channel_data));
      auth = this.credentials.key + ':' + crypto.createHmac('sha256', this.credentials.secret).update(stringToSign).digest('hex');
      req = {
        id: uuid.v1(),
    ...
    PusherClient.prototype.resetActivityCheck = function() {
      ...
      return this.activityTimeout = setTimeout(function() {
        logger("info", "Pusher", "Pinging Pusher for activity - " + ((new Date).toLocaleTimeString()));
        ...
        return _this.waitingTimeout = setTimeout(function() {
          logger("warning", "Pusher", "Disconnecting due to inactivity at " + ((new Date).toLocaleTimeString()));
          _(_this.channels).each(function(channel) {
            return _this.unsubscribe(channel.channel_name, channel.channel_data);
          });
          logger("info", "Pusher", "Connecting again - " + ((new Date).toLocaleTimeString()));
    ...
    PusherClient.prototype.connect = function() {
      ...
      this.client.on('connect', function(connection) {
        logger("success", "Pusher", "Connected to Pusher!");
        _this.connection = connection;
        logger("info", "Pusher", "Connection state: "+_this.connection.state);
      ...
      logger("info", "Pusher", "trying connecting to pusher on - wss://ws.pusherapp.com:443/app/" + this.credentials.key + "?client=node-pusher-server&version=0.0.1&protocol=5&flash=false");
    ...
    PusherClient.prototype.recieveMessage = function(msg) {
      ...
      if (msg.type === 'utf8') {
        ...
        if (payload.event === "pusher:connection_established") {
          ...
          logger("info", "Pusher", "Socket ID: "+this.state.socket_id);
          this.emit('connect');
        }
        ...
        channel = this.channels[payload.channel];
        logger("info", "Pusher", "Recieved event: " + payload.event);
        if (payload.event === "pusher:error") {
          logger("warning", "Pusher", "Recieved error:");
          console.log(payload);
        }
        if (channel) {
